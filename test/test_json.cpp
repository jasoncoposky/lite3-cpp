...
void test_empty_object_to_json() {
  ...
  if (json_str == "{}") {
    ...
    std::cout << "test_empty_object_to_json FAILED. Expected '{}', got '"
              << json_str << "'" << std::endl;
  }
}
...
void test_empty_object_from_json() {
  ...
  if (buffer.size() == lite3cpp::config::node_size) {
    std::cout << "test_empty_object_from_json passed." << std::endl;
  } else {
    std::cout << "test_empty_object_from_json FAILED. Expected buffer size "
              << lite3cpp::config::node_size << ", got " << buffer.size()
              << std::endl;
  }
}
...
void test_string_object_to_json() {
  ...
  if (json_str == expected_str) {
    ...
    std::cout << "test_string_object_to_json FAILED. Expected '" << expected_str
              << "', got '" << json_str << "'" << std::endl;
  }
}
...
void test_string_object_from_json() {
  ...
  if (buffer.size() > lite3cpp::config::node_size) {
    ...
    std::cout << "test_string_object_from_json FAILED. Buffer size not greater "
                 "than initial."
              << std::endl;
  }
}
