...
public:
  bool log(lite3cpp::LogLevel level, std::string_view message,
           std::string_view operation, std::chrono::microseconds duration,
           size_t buffer_offset, std::string_view key) override {
...
public:
  ...
  bool record_bytes_received(size_t bytes) override {
    std::cout << "Metric: bytes received: " << bytes << std::endl;
    return true;
  }
  bool record_bytes_sent(size_t bytes) override {
    std::cout << "Metric: bytes sent: " << bytes << std::endl;
    return true;
  }
...
int main() {
  ...
  std::cout << "--- Initial logging attempts (should show nothing unless "
               "library calls log) ---"
            << std::endl;
  // These calls will be filtered out by the default LogLevel::Info threshold
  lite3cpp::log_if_enabled(lite3cpp::LogLevel::Debug, "Debug message",
                           "example", std::chrono::microseconds(10), 0);
  lite3cpp::log_if_enabled(lite3cpp::LogLevel::Info, "Info message", "example",
                           std::chrono::microseconds(20), 0);
  ...
  lite3cpp::log_if_enabled(lite3cpp::LogLevel::Debug,
                           "Debug message (filtered)", "example",
                           std::chrono::microseconds(30), 0);
  std::cout << "Attempting to log Info (should be filtered)" << std::endl;
  lite3cpp::log_if_enabled(lite3cpp::LogLevel::Info, "Info message (filtered)",
                           "example", std::chrono::microseconds(40), 0);
  std::cout << "Attempting to log Warn (should pass)" << std::endl;
  lite3cpp::log_if_enabled(lite3cpp::LogLevel::Warn, "Warning message (passed)",
                           "example", std::chrono::microseconds(50), 0);
  ...
  lite3cpp::log_if_enabled(lite3cpp::LogLevel::Debug, "Debug message (passed)",
                           "example", std::chrono::microseconds(60), 0);
  std::cout << "Attempting to log Info (should pass)" << std::endl;
  lite3cpp::log_if_enabled(lite3cpp::LogLevel::Info, "Info message (passed)",
                           "example", std::chrono::microseconds(70), 0);
